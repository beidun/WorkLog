...
- 启用 Provider 后由会话 Agent 主导语义判断，并输出带 event ID 的原子事实；确定性规则只负责初始基线、证据门控和模型失败时的回退
- 识别 Codex `commentary/final_answer`，进行中的对话不会因中途测试通过而提前标记完成
- 优先采用最近一条有效需求作为工作目标，忽略“开始”“继续”和会话续接提示
- 可选 LLM 增强会话语义；启用后 Agent 主导目标、进展、事实和下一步，模型失败、超时或引用非法时自动回退到确定性 Digest
...
- 项目详情提供“重新分析”按钮，可只重跑当前项目的 Agent；分析期间显示进行中状态，完成后自动刷新项目结论
...
bun src/cli.ts scan-project base-data-spiders # 只将指定项目交给 Agent，其他项目只做本地索引
...
| `POST` | `/api/projects/:id/analyze` | 只重跑指定项目的 Agent 分析；其他项目不会发送给 Provider |
...
未启用 Provider 时，`SessionDigest`、标题合并、状态和项目摘要使用确定性规则，保证本地可复现且无需模型密钥。启用 Provider 后，会话 Agent 是语义主路径：它负责理解目标、事实、进展、验证、阻塞和下一步，并返回带真实 event ID 的原子 `facts`；确定性结果仅作为基线、状态安全门和模型失败/超额时的回退。摘要用事件内容与 Provider 配置哈希做增量缓存，只有变化过的会话或尚未进入增强额度的会话才会重建。首页会显示各层 Agent 的增强覆盖数，未覆盖对象继续使用确定性结果，并在后续扫描按优先级进入 deferred 队列。
